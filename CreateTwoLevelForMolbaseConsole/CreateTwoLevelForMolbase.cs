...
namespace CreateTwoLevelForMolbaseConsole
{
    class CreateTwoLevelForMolbase
    {
        static void Main(string[] args)
        {
            var regex = new Regex(@"(?<=\\)(\d{1,7})-\d{1,2}-\d\.(?:png|jpg|gif)");

            int casRn_part1;
            string path_root, path_second, directory, destination;
            foreach (var item in Directory.GetDirectories(@"F:\molbase\"))
            {
